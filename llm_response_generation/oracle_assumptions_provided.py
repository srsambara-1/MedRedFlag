#!/usr/bin/env python3
"""Oracle Assumptions Provided: Model is given ground truth false assumptions, then responds."""
...
def strategy_oracle(patient_question, presuppositions):
    return f"""
...
def run_strategy_oracle(client, model, patient_question, presuppositions, api_type):
    prompt = strategy_oracle(patient_question, presuppositions)
    ...
    if not content:
        return {"response": None, "error": "Empty completion returned by API"}
    ...
    if parse_err:
        return {"response": None, "think": think_text, "error": f"JSON parse error: {parse_err}", "raw_output": content}

    return {"response": result.get("response"), "think": think_text}
...
def main():
    ...
    parser.add_argument("--output-column", default=None, help="Column name for the model's response")
    ...
    output_column = args.output_column or f"{model_slug}_oracleAssumptionsProvided"
    ...
    for idx, row in df.iterrows():
        ...
        presuppositions = str(row.get(args.assumption_column, ""))

        print(f"[{idx+1}/{len(df)}] Processing {rid}")

        result = run_strategy_oracle(client, args.model, patient_question, presuppositions, args.api_type)
